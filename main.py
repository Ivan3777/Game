...
def check_city():
    ...
    if not city:
        ...
        response = f'Місто {city} відсутнє в списку який знає Джонсонюк'
    ...
    game_log.insert(tk.END, f'Джонсонюк: {response}\n')
...
def surrender():
    game_log.config(state=tk.NORMAL)
    game_log.insert(tk.END, f'Джонсонюк виграв!\n')
    game_log.yview(tk.END)
    game_log.config(state=tk.DISABLED)
    notification.config(
        text='Джонсонюк виграв! Каже: Добрий день Everybody :)')
    entry.delete(0, tk.END)
...
left_frame.pack(side=tk.LEFT, padx=20, pady=20, fil=tk.BOTH)
...
button_end = tk.Button(button_frame, text='Джонсонюк Win', command=surrender)
